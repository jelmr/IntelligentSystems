 ...
 *         Date: 22/01/14
 */
public class NeuralBot15 extends DarwinBot15 {

	public static final Bot15[] BOTS = new Bot15[]{new CompetitionBot15(), new BullyBot15(), new CarnageBot15(), new Empty15(), new CustomBot15()};
	...
	public NeuralBot15() {
		int inputSize = 7;
		int hiddenSize = 9;
		int outputSize = 2* Heuristic15.HEURISTICS.length+2;
...
//			weights[i] = Math.random();
	...
	public NeuralBot15(double[] weights) {
	...
	public Action15 getAction(PlanetWars15 pw) {
		...
		int friendlyShipCount = pw.NumShips(Bot15.FRIENDLY);
		int hostileShipCount = pw.NumShips(Bot15.HOSTILE);
		int friendlyGrowth = SimulatedPlanetWars15.totalGrowth(pw, Bot15.FRIENDLY);
		int hostileGrowth = SimulatedPlanetWars15.totalGrowth(pw, Bot15.HOSTILE);

		int heuristicCount = Heuristic15.HEURISTICS.length;

		NeuralNetwork15 network = new NeuralNetwork15(9, heuristicCount *2+2,friendlyPlanetCount, hostilePlanetCount, neutralPlanetCount,
				friendlyShipCount, hostileShipCount, friendlyGrowth, hostileGrowth);

		network.setWeights(weights);

		double bestValue = 0;
		Heuristic15 bestSource = null;
		...
		for (int i = 0; i < heuristicCount; i++) {
			if (result[i] > bestValue) {
				bestValue = result[i];
				bestSource = Heuristic15.HEURISTICS[i];
			}
		}
		...
		bestValue = 0;
		Heuristic15 bestTarget = null;

		for (int i = 0; i < heuristicCount; i++) {
			if (result[i+heuristicCount] > bestValue) {
				bestValue = result[i+heuristicCount];
				bestTarget = Heuristic15.HEURISTICS[i];
			}
		}

		List<Planet15> planets = null;
		...
		Planet15 source = Heuristic15.select(pw.MyPlanets(), bestSource);
		Planet15 target = Heuristic15.select(planets, bestTarget);
	...
	@Override
	public DarwinBot15 getInstance(double... pars) {
		return new NeuralBot15(pars);
	}


	public Bot15 getBot(NeuralNetwork15 network) {
		double bestValue = 0;
		Bot15 bestBot = null;
